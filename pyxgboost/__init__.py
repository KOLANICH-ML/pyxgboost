...
def missingCheck(check: ast.Compare, el: ast.Subscript, default: bool) -> ast.BoolOp:
	...
	return ast.BoolOp(op=(ast.Or() if default else ast.And()), values=[ast.Compare(left=el, ops=[(ast.Is() if default else ast.IsNot())], comparators=[noneCnst]), check])
...
def tanhCall(arg):
	return ast.Call(func=ast.Name(tanhName), args=[arg], keywords=[])
...
def generateSplit(varId: int, margin: float, les: ast.stmt, gret: ast.stmt, defaultLeft: bool, useSigmoids: bool) -> ast.If:
	...
	if not useSigmoids:
		check = ast.Compare(left=el, comparators=[astNum(margin)], ops=[ast.Lt()])
...
def genModelSummary(model: Xgboost) -> str:
	...
	if model.param.contain_extra_attrs:
		res.extend((p.pair[0].str + ": " + p.pair[1].str for p in model.attributes_.data))
	return "\n".join(res)
...
def makeGroup(name, base_score, useNumPy=None):
	...
	if not useNumPy:
		...
		code.append(
			ast.For(
				body=[
					ast.AugAssign(target=res, op=ast.Add(), value=treeCall)
				],
				orelse=[],
				**iterArgDict,
			)
